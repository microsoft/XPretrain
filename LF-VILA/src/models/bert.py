...
class BertAttention(nn.Module):
    def __init__(self, args, config, position_embedding_type=None):
        super().__init__()
    ...
    def forward(
        ...
        if not self.args.deepspeed_sparse_attention:
            ...
            outputs = (attention_output,) + (None,)  # add attentions if we output them
        return outputs
...
class BertLayer(nn.Module):
    def __init__(self, args, config):
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(args, config)
        self.is_decoder = config.is_decoder
...
class BertEncoder(nn.Module):
    def __init__(self, config, args, sparse_attention_config=None):
        ...
        if config.stage == 1:
            self.layer = nn.ModuleList([
                copy.deepcopy(layer) for _ in range(config.stage1_layers)
            ])
        else:
            self.layer = nn.ModuleList([
                copy.deepcopy(layer) for _ in range(config.num_hidden_layers)
            ])
        self.stage0 = list(range(0, config.num_local_layers))
        self.stage1 = list(range(config.num_local_layers, int(config.stage1_layers)))
        self.stage2 = list(range(int(config.stage1_layers), config.num_hidden_layers))
    ...
    def forward(
        ...
        if stage == 0:
            stage_layers = self.stage0
        elif stage == 1:
            stage_layers = self.stage1
        else:
            stage_layers = self.stage2
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, args, config, add_pooling_layer=True):
    ...
    def _freeze_stages(self):
        ...
        if self.config.bert_frozen_stage >= 0:
            self.embeddings.eval()
            for param in self.embeddings.parameters():
                param.requires_grad = False
        if self.config.bert_frozen_stage >= 1:
            for i in range(0, self.config.bert_frozen_stage):
                m = self.encoder.layer[i]
                m.eval()
    ...
    def forward(
        ...
        if self.sparse_attention_config is not None and pad_len > 0:
            encoder_outputs[-1] = self.sparse_attention_utils.unpad_sequence_output(
                pad_len, encoder_outputs[-1])
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, args, config):
        ...
        self.bert = BertModel(args, config, add_pooling_layer=False)
