...
from src.models import LFVILA_Video_Classification

from src.tools import Trainer_Video_Classification
...
def main():
    ...
    model = LFVILA_Video_Classification(args, config)
    ...
    dataloader_train = dataloader_trains['VideoClassificationDataset-train']
    ...
    trainer = Trainer_Video_Classification(args, config, model_engine, optimizer, scheduler, dataloader_train, dataloader_vals['VideoClassificationDataset-val'])

    LOGGER.info('start first evaluate')

    trainer.evaluate(dataloader_vals['VideoClassificationDataset-val'])
    
    if not config.TRAINING.only_val:
