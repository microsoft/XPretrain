...
from src.modeling.modeling_stage import (
    HDVILAForSequenceClassification,
    HDVILAForMultipleChoice,
    HDVILAForRegression)
# from src.modeling.e2e_model import HDVILA
from src.modeling.e2e_model import HDVILA

from src.datasets.dataset_video_qa import HDVILAVideoQADataset, VideoQACollator
...
from src.optimization.sched import AutoStep
...
def setup_model(cfg, device=None):
    ...
    if hasattr(cfg, "pixel_random_sampling_size"):
        setattr(model_cfg, "pixel_random_sampling_size", cfg["pixel_random_sampling_size"])

    if cfg.task in ["action", "transition"]:
        transformer_model_cls = HDVILAForMultipleChoice
    else:
        transformer_model_cls = HDVILAForSequenceClassification
...
def validate(model, val_loader, cfg, train_global_step, eval_score=True):
    ...
    for val_step, batch in enumerate(val_loader):
        ...
        #     losses = torch.gather(out, -1, labels.view(-1, 1))
        # else:
        #     _, losses = model.transformer.calc_loss(logits, labels)
        #
...
def start_training(cfg):
    ...
    if hasattr(cfg, "auto_step"):
        auto_step = AutoStep(cfg.auto_step["tolerance"], cfg.auto_step["gamma"])
    ...
    for step, batch in enumerate(InfiniteIterator(train_loader)):
        if hasattr(cfg, 'save_images') and (step == 0) and (hvd.rank() == 0):
            other_frames = batch["img_other"] / 255.
            other_frames = other_frames.reshape(-1, other_frames.shape[-3], other_frames.shape[-2], other_frames.shape[-1])
            middle_frames = batch["img_middle"] / 255.
            middle_frames = middle_frames.reshape(-1, middle_frames.shape[-3], middle_frames.shape[-2],
                                                middle_frames.shape[-1])
            os.makedirs(join(cfg.output_dir, 'image'), exist_ok=True)
            tutils.save_image(other_frames, join(cfg.output_dir, 'image/other_frames.png'), nrow=4,
                              normalize=True, range=(0.0, 1.0))
            tutils.save_image(middle_frames, join(cfg.output_dir, 'image/middle_frames.png'), nrow=4,
                              normalize=True, range=(0.0, 1.0))
            LOGGER.info(f'Step {global_step}: Image saved')
        ...
        if pool_method == "lse":
            out = torch.logsumexp(logits.view(logits.shape[0], -1), dim=-1, keepdim=True) \
                - torch.logsumexp(logits, dim=1)
            loss = torch.gather(out, -1, labels.view(-1, 1))
        else:
            if hasattr(cfg, "temperature"):
                logits = logits / cfg.temperature
            _, loss = model.transformer.calc_loss(logits, labels)
        loss = loss.mean()
        ...
        if (step + 1) % cfg.gradient_accumulation_steps == 0:
            ...
            if hasattr(cfg, "auto_step"):
                lr_this_step_transformer = auto_step.get_lr(global_step,cfg.learning_rate,cfg.num_train_steps,warmup_ratio=cfg.warmup_ratio)
                lr_this_step_cnn = auto_step.get_lr(global_step,cfg.cnn_learning_rate,cfg.num_train_steps,warmup_ratio=cfg.warmup_ratio)
                lr_this_step_align = auto_step.get_lr(global_step,cfg.align_learning_rate,cfg.num_train_steps,warmup_ratio=cfg.warmup_ratio)
            ...
            if global_step % cfg.valid_steps == 0:
                ...
                overall_acc = qa_scores["overall_acc"]
                if overall_acc > best_acc:
                    best_step = global_step
                    best_acc = overall_acc

                if hasattr(cfg, "auto_step"):
                    auto_step.step(overall_acc)
...
def data_mount(cfg):
    ...
    for db in cfg.train_datasets:
        for key in db.txt:
            db.txt[key] = os.path.join(cfg.data_mount_dir, db.txt[key])
...
if __name__ == '__main__':
    ...
    if input_cfg.do_inference:
        ...
        global_step, best_step = start_training(input_cfg)
        hvd.barrier()
        
        setattr(input_cfg, "do_inference", True)

        input_cfg.inference_model_step = global_step
        start_inference(input_cfg)
        hvd.barrier()

        input_cfg.inference_model_step = best_step
        start_inference(input_cfg)
        hvd.barrier()