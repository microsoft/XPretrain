...
class TimeSformer(nn.Module):
    """ Vision Transformere
    """
    def __init__(self, depth=12, num_frames=7, H=10, W=16, embed_dim=768,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0.1, norm_layer=partial(nn.LayerNorm, eps=1e-6), attention_type='divided_space_time', timesformer_type='new', dropout=0.):
        super().__init__()
        self.timesformer_type = timesformer_type
        print("use timesformer type: ", timesformer_type)
        self.attention_type = attention_type
        self.depth = depth
        self.dropout = nn.Dropout(dropout)
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
    ...
    def forward(self, x):
        ...
        if H != self.H or W != self.W:
            ...
            other_pos_embed = other_pos_embed.reshape(1, x.size(2), self.H, self.W)
            new_pos_embed = F.interpolate(other_pos_embed, size=(H, W), mode='bilinear')
            new_pos_embed = new_pos_embed.flatten(2)
            new_pos_embed = new_pos_embed.transpose(1, 2)
            x = x + new_pos_embed
        else:
        ...
        if self.attention_type != 'space_only':
            ...
            if T != self.time_embed.size(1):
                time_embed = self.time_embed.transpose(1, 2)
                new_time_embed = F.interpolate(time_embed, size=(T), mode='linear')
                new_time_embed = new_time_embed.transpose(1, 2)
                x = x + new_time_embed
            else:
                x = x + self.time_embed
        ...
        if self.attention_type == 'space_only':
            ...
            x = x.reshape(B, H, W, T, C).permute(0, 3, 4, 1, 2)
        return x


if __name__ == "__main__":
    model = TimeSformer()
    # input = torch.randn(2, 7, 768, 10, 16)
    input = torch.randn(2, 5, 768, 8, 10)
    output = model(input)
    print(output.shape)
