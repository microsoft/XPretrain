...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, "bert_frozen_stage"):
            self.frozen_stages = config.bert_frozen_stage
        else:
            self.frozen_stages = []
        if config.stage == 1:
        ...
        self.stage1 = list(range(0, int(config.num_hidden_layers/2)))
        self.stage2 = list(range(int(config.num_hidden_layers/2), config.num_hidden_layers))
        self._freeze_stages()
    ...
    def _freeze_stages(self):
        if isinstance(self.frozen_stages, int):
        ...
        elif isinstance(self.frozen_stages, list):
            ...
            raise NotImplementedError(f"Such Bert freeze type {type(self.frozen_stages)} not implemented!")
