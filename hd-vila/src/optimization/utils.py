...
def setup_e2e_optimizer(model, opts):
    ...
    cnn_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if ("cnn" in n or "grid_encoder" in n) and p.requires_grad]
    align_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if "timesformer" in n and p.requires_grad]
    ...
    align_grouped_parameters = build_e2e_optimizer_w_lr_mul(
        align_param_optimizer,
        opts.align_learning_rate, opts.align_weight_decay)
    ...
    optimizer_grouped_parameters.extend(align_grouped_parameters)
...
def build_e2e_optimizer_w_lr_mul(
    ...
    no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight', 'embed']
    # Prepare optimizer
    if lr_mul_prefix == "":
        param_optimizer = model_param_optimizer
        param_top = []
